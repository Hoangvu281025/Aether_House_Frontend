...
const Header = () => {
  ...
  useEffect(() => {
    const onDocClick = (e) => {
      ...
      if (!avatarRef.current.contains(e.target)) setIsAvatarOpen(false);
    };
    document.addEventListener("mousedown", onDocClick);
    return () => document.removeEventListener("mousedown", onDocClick);
  }, []);

  // đóng khi chuyển route / mở menu / mở cart / nhấn ESC
  useEffect(
    () => setIsAvatarOpen(false),
    [location.pathname, isMenuOpen, isCartOpen]
  );
  useEffect(() => {
    const onKey = (e) => e.key === "Escape" && setIsAvatarOpen(false);
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((e) => {
        ...
        console.error(e);
        const msg =
          e?.response?.data?.message || e?.message || "Cannot load menu";
        setErr(msg);
      })
      ...
      >
        ...
        <div className="header_col2">
          <div className="header_search">
            {/* ⬇️ chỉ thay input thành form để Enter điều hướng */}
            <form
              onSubmit={(e) => {
                e.preventDefault();
                ...
                if (keyword) {
                  ...
                  setQ(""); // clear sau khi bấm Enter (tùy thích)
                }
              }}
              autoComplete="off"
            >
          ...
          <div className="headers_icons">
            ...
            {user ? (
              <div className="hdr-acc-wrap" ref={avatarRef}>
                <button
                  type="button"
                  className="header_avatar_btn"
                  title="Your account"
                  aria-haspopup="menu"
                ...
                <div
                  className={`hdr-dropdown ${isAvatarOpen ? "open" : ""}`}
                  role="menu"
                >
                  <Link
                    to="/userprofile?tab=account"
                    className="hd-item"
                    role="menuitem"
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        navigate("/"); // 🔹 chỉ đổi route trong app, không reload cứng
            ...
            <div className="bag_wrap" onClick={toggleCart}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="header_icon"
                viewBox="0 0 24 24"
              ...
              {bagCount > 0 && <span className="bag_badge">{bagCount}</span>}
            </div>
